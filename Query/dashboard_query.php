...
function getPoRequests($conn, $mgrCode)
{
    $query = "
        ...
        JOIN mast_branch mb ON mb.BR_MANAGER_CODE = po.MGR_CODE  
        WHERE po.REQ_PO_STS = '001' 
          AND po.MGR_CODE = ?  
...
function getApprovedCount($conn, $mgrCode)
{
    // Initialize $branchCode and count to default values
    $branchCode = null;
    $approvedCount = 0;
    ...
    if ($stmt = $conn->prepare($branchQuery)) {
        ...
        // Check if a valid branch code was retrieved
        if ($branchCode) {
            // Query to get the count for approved POs for the branch and current month
            $countQuery = "
                SELECT 
            ...
            if ($stmt = $conn->prepare($countQuery)) {
                $stmt->bind_param("sii", $branchCode, $currentMonth, $currentYear);  // Bind branch code, month, and year
                $stmt->execute();
                $stmt->bind_result($approvedCount);  // Fetch the count
                $stmt->fetch();
                $stmt->close();

                // Return the approved count
                return $approvedCount ? $approvedCount : 0;
            }
        }
    }
